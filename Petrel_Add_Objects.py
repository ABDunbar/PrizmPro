...
pwr_description = WorkflowDescription(
    name="Horizon - outliers correction",
...
pwr_description.add_integer_parameter(
    name="size",
    label="Define the moving window size",
    description="Using a moving window we compute the  local mean and standard deviation for each point in the horizon array.",
    default_value=40,
    minimum_value=2,
    maximum_value=1000,
)
pwr_description.add_float_parameter(
    name="threshold",
    label="Define threshold value",
    description="A point is considered an outlier if its difference from the local mean is greater than threshold times the local standard deviation. The threshold determines how strict or lenient the outlier detection is.",
    default_value=1,
    minimum_value=0,
    maximum_value=1000,
)
pwr_description.add_integer_parameter(
    name="nws",
    label="Define the neighborhood size",
    description="The identified outliers are corrected by replacing them with a weighted average of their neighbors. The neighborhood size for calculating the weighted average is defined by the indices in the slicing operation.\nNote that choice of the neighborhood window size (6x6 in this case) and its positioning relative to the outlier can affect the correction result. Adjusting these parameters might provide different outcomes depending on the specific characteristics of the data and the desired correction behavior.",
    default_value=10,
    minimum_value=2,
    maximum_value=1000,
)
pwr_description.add_integer_parameter(
    name="iterations",
    label="Define the number of iterations",
    description="The approach is to iteratively identify and correct outliers until a specified number of iterations are reached.\nWhy iteratively? Seismic horizons can have various scales of irregularities. Some might be genuine features, while others could be due to noise or inaccuracies in the interpretation process. By performing the correction iteratively, the approach tries to address both large-scale and fine-scale outliers, refining the horizon in each pass.",
    default_value=3,
    minimum_value=1,
    maximum_value=1000,
)
